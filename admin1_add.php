...
<body>
    ...
    <div id="info1">
        <div id="info_nav1">
            <ul class="nav1">
                <li><a href="http://localhost/admin1_add.php">Добавить новость</a></li>
                <li><a href="http://localhost/admin1_update.php">Редактировать новость</a></li>
                <li><a href="http://localhost/admin1_delete.php">Удалить новость</a></li>
        ...
        <div id="info_text2">
            ...
            <?php
                ...
                $link = mysqli_connect($host, $user, $password, $database)
                    ...
                    echo "<p>Звполните все поля новости, которую хотите добавить</p>";
                    echo "<form action='' method='POST'>";
                    echo "<input type='title' placeholder='Заголовок' name='title' required pattern='.+'>";
                    echo "<input type='information' placeholder='Текст новости' name='information' required pattern='.+'>";
                    echo "<input type='date' id='date' name='date_time' />";
                    echo "<input type='photo' placeholder='Ссылка на фото' name='photo' required pattern='.+'>";
                    echo "<input type='submit' value='Отправить'>";
                    echo "</form>";

                    if (isset($_POST['title'], $_POST['information'], $_POST['date_time'], $_POST['photo'])) {
                        require 'connection.php';
                        $link = mysqli_connect($host, $user, $password, $database)
                            or die("Ошибка " . mysqli_error($link));
                
                                    $resultt = mysqli_query($link, "SELECT * FROM news_content ");
                        ...
                        if (!empty($_POST['title']) && !empty($_POST['information']) && !empty($_POST['date_time']) && !empty($_POST['photo'])) {
                                $title = htmlentities(mysqli_real_escape_string($link, $_POST['title']));
                                $information = htmlentities(mysqli_real_escape_string($link, $_POST['information']));
                                ...
                                $query = "INSERT INTO news_content VALUES($rows,'$title','$information','$date_time','$photo','1')";

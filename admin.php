<?php
    if (isset($_POST['login'], $_POST['password'])) {
        ...
        if (!empty($_POST['login']) && !empty($_POST['password'])) {
            //добавить проверки какие-нибудь
            $login = htmlentities(mysqli_real_escape_string($link, $_POST['login']));
            $password = htmlentities(mysqli_real_escape_string($link, $_POST['password']));

            $result = mysqli_query($link, "SELECT password FROM user WHERE login='$login'");
            $myrow = mysqli_fetch_array($result);

            $result2 = mysqli_query($link, "SELECT admin FROM user WHERE login='$login'");
            ...
            if (!empty($myrow['password'])) {
                ...
                window.onload = function () { alert("Вход выполнен успешно"); }
                
                </script>';
                        $new_url = 'http://localhost/admin1.php';
                    ...
                    window.onload = function () { alert("Вход выполнен успешно"); }
                        ...
                        $new_url = 'http://localhost/admin2.php';
                        header('Location: ' . $new_url);
                    }
                } else {
                ...
                window.onload = function () { alert("Введенный пароль неверный"); }
                ...
                window.onload = function () { alert("Введенный логин неверный"); }
            ...
            window.onload = function () { alert("Заполните все поля"); }
...
<body>
    ...
    <div id="info1_ad">
        ...
        <div id="info_text2_ad">
        <h3>
            Вход для администратора
        </h3>
            <div class="form-inner">
                <form action="" method="POST">
                    <input type="text" placeholder="login" name="login" required pattern=".+">
                    <input type="password" placeholder="password" name="password" required pattern=".+">

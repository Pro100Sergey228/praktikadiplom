...
<body>
    ...
    <div id="info1">
        <div id="info_nav1">
            <ul class="nav1">
            <li><a href="http://localhost/admin2_add.php">Добавить запись в расписание</a></li>
                <li><a href="http://localhost/admin2_update.php">Редактировать запись в расписании</a></li>
                <li><a href="http://localhost/admin2_delete.php">Удалить запись из расписания</a></li>
                <li><a href="http://localhost/admin.php">Выйти</a></li>
            </ul>
        </div>
        <div id="info_text2">
            <h3>АДМИН 2</h3>
            ...
            <?php
                ...
                $query = mysqli_query($link, "SELECT id FROM schedule");
                if ($query) {
                    $rows = mysqli_num_rows($query); // количество полученных строк
                    $columns = mysqli_num_fields($query); // количество полученных строк

                    echo "<p>Заполните все поля новости, которую хотите изменить</p>";
                echo "<form action='' method='POST'>";
                    ...
                    echo "<input type='text' placeholder='Статус 1/2' name='title' required pattern='.+'>";
                    echo "<input type='date' id='date' name='date_time' />";
                    echo "<input type='time' placeholder='11:00:00' name='time' required pattern='.+'>";
                    echo "<input type='text' placeholder='Кабинет' name='cabinet' required pattern='.+'>";
                    echo "<input type='submit' value='Отправить'>";
                    echo "</form>";

                    if (isset($_POST['theme'], $_POST['title'], $_POST['date_time'], $_POST['time'],  $_POST['cabinet'])) {
                        ...
                        $cabinet = htmlentities($_POST['cabinet']);
                
                
                        if (!empty($_POST['theme']) && !empty($_POST['title']) && !empty($_POST['date_time']) && !empty($_POST['time'])  && !empty($_POST['cabinet'])) {
                                $theme = htmlentities(mysqli_real_escape_string($link, $_POST['theme']));
                                $title = htmlentities(mysqli_real_escape_string($link, $_POST['title']));
                                $date_time = htmlentities(mysqli_real_escape_string($link, $_POST['date_time']));
                                ...
                                $query = "UPDATE schedule SET id='$theme', status='$title', date1='$date_time', time='$time',  cabinet='$cabinet' WHERE id='$theme'";
                                $result1 = mysqli_query($link, $query) or die("Ошибка " . mysqli_error($link));
                                if ($result1) {
                    echo '<script ="text/javascript"> window.onload = function () { alert("Успешно!"); }</script>';

                                } else {
                echo '<script ="text/javascript"> window.onload = function () { alert("Упс! Попробуйте снова!"); }</script>';

    ...
    <div id="info1">
        ...
        <div id="info_text2">
            ...
            <?php
                ...
                $query = mysqli_query($link, "SELECT * FROM news_content");
                if ($query) {
                    $rows = mysqli_num_rows($query); // количество полученных строк
                    $columns = mysqli_num_fields($query); // количество полученных строк

                    echo "<table><tr><th>Номер</th><th>Название</th><th>Текст новости</th><th>Дата публикации</th><th>Картинка</th></tr>";
                    for ($i = 0; $i < $rows; ++$i) {
                        $row = mysqli_fetch_row($query);
                        echo "<tr>";
                        for ($j = 0; $j < $columns-1; ++$j) echo "<td>$row[$j]</td>";
                        echo "</tr>";
                    }
                    echo "</table>";
                    // очищаем результат

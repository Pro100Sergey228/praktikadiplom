...
<body>
        ...
        <div id="info_text2">
            ...
            <div class="form-inner">
                <form action="" method="POST">
                    <input type="text" placeholder="ФИО" name="fio1" required pattern=".+">
                    <input type="text" placeholder="E-mail" name="email1" required pattern="\w+@[a-zA-Z]+?\.[a-zA-Z]{2,4}">
                    <input type="text" placeholder="Контактный телефон" name="phone1" required pattern="(8|\+7)\d{3}[\-]\d{3}[\-]\d{2}[\-]\d{2}|(8|\+7)\d{10}">
                    <input type="text" placeholder="Почтовый адрес" name="adress1" required pattern=".+">
                    <input type="text" placeholder="Тема обращения" name="theme1" required pattern=".+">
                    <textarea placeholder="Сообщение..." rows="3" name="information1"></textarea>
                    <p>Напоминаем, что отправка электронного документа с использованием сайтов, электронных почтовых адресов, принадлежащих другим лицам (через опосредованных лиц), не гарантирует защиту Ваших персональных данных и неразглашение сведений, содержащихся в обращении и касающихся Вашей частной жизни. С учетом изложенного предлагаем Вам обратиться с интересующим вопросом лично или письменно.</p>
    ...
    <?php
    if (isset($_POST['fio1'], $_POST['email1'], $_POST['phone1'], $_POST['adress1'], $_POST['theme1'], $_POST['information1'])) {
        require 'connection.php';
        $link = mysqli_connect($host, $user, $password, $database)
            or die("Ошибка " . mysqli_error($link));

        $resultt = mysqli_query($link, "SELECT * FROM reviews_form ");
        ...
        $fio1 = htmlentities($_POST['fio1']);
        $email1 = htmlentities($_POST['email1']);
        $phone1 = htmlentities($_POST['phone1']);
        $adress1 = htmlentities($_POST['adress1']);
        $theme1 = htmlentities($_POST['theme1']);
        $information1 = htmlentities($_POST['information1']);

        if (!empty($_POST['fio1']) && !empty($_POST['email1']) && !empty($_POST['phone1']) && !empty($_POST['adress1']) && !empty($_POST['theme1']) && !empty($_POST['information1'])) {
            $fio1 = htmlentities(mysqli_real_escape_string($link, $_POST['fio1']));
            $email1 = htmlentities(mysqli_real_escape_string($link, $_POST['email1']));
            $phone1 = htmlentities(mysqli_real_escape_string($link, $_POST['phone1']));
            $adress1 = htmlentities(mysqli_real_escape_string($link, $_POST['adress1']));
            $theme1 = htmlentities(mysqli_real_escape_string($link, $_POST['theme1']));
            $information1 = htmlentities(mysqli_real_escape_string($link, $_POST['information1']));

            $query = "INSERT INTO reviews_form VALUES($rows,'$fio1','$email1','$phone1','$adress1','$theme1','$information1','не обработано','2')";
            $result1 = mysqli_query($link, $query) or die("Ошибка " . mysqli_error($link));
            if ($result1) {
                echo '<script ="text/javascript">

    window.onload = function () { alert("Ваше сообщение успешно отправлено!"); }
    ...
    window.onload = function () { alert("Ваше сообщение не было отправлено! Проверьте правильность заполнения формы!"); }

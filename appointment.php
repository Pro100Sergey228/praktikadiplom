<?php
ob_start();
    if (isset($_POST['theme1'], $_POST['date1'], $_POST['fio1'], $_POST['email1'], $_POST['phone1'])) {
        ...
        $resultt = mysqli_query($link, "SELECT * FROM appointment_form");
        $resultt1 = mysqli_query($link, "SELECT * FROM schedule WHERE date1='$date1'");
        if ($resultt1) {
            $rows1 = mysqli_num_rows($resultt1);
        }
        ...
        if (!empty($_POST['theme1']) && !empty($_POST['date1']) && !empty($_POST['fio1']) && !empty($_POST['email1']) && !empty($_POST['phone1'])) {
            if ($rows1 > 0) {
                $theme1 = htmlentities(mysqli_real_escape_string($link, $_POST['theme1']));
                $date1 = htmlentities(mysqli_real_escape_string($link, $_POST['date1']));
                ...
                $query = "INSERT INTO appointment_form VALUES($rows,'$theme1','$date1','','$fio1','$email1','$phone1','2')";
                $result1 = mysqli_query($link, $query) or die("Ошибка " . mysqli_error($link));
                if ($result1) {
                    $new_url = 'http://localhost/appointment2.php';
                    header('Location: '.$new_url);
                    exit();
                } else {
                    echo '<script ="text/javascript"> window.onload = function () { alert("Запись на данную дату не доступна! Выберите другую дату!"); }</script>';
                }
            } else {
                echo '<script ="text/javascript"> window.onload = function () { alert("Запись на данную дату не доступна! Выберите другую дату!"); }</script>';
            }
        } else {
            echo '<script ="text/javascript"> window.onload = function () { alert("Запись на данную дату не доступна! Проверьте правильность заполнения формы!"); }</script>';
        }
...
<body>
    ...
    <div id="info1">
        ...
        <div id="info_text2">
            <h3>Запись на прием г. Фролово</h3>
            <div class="form-inner">
                <form action="" method="POST">
                    <select id="theme" name="theme1">
                        <option value="Не указано">Не указано</option>
                        <option value="Льготы">Льготы</option>
                        <option value="Пособия на детей">Пособия на детей</option>
                        <option value="Пособия малоимущим гражданам">Пособия малоимущим гражданам</option>
                    </select>

...
<body>
    ...
    <div id="info1">
        ...
        <div id="info_text2">
            ...
            <?php
                ...
                $link = mysqli_connect($host, $user, $password, $database)
                    ...
                    $query1 = mysqli_query($link, "SELECT * FROM schedule");
                    if ($query1) {
                        $rows = mysqli_num_rows($query1); // количество полученных строк
                        $columns = mysqli_num_fields($query1); // количество полученных строк
    
                    echo "<h3>Актуальные отзывы</h3>";
    
                        echo "<table><tr><th>Номер записи</th><th>Статус записи 1/2</th><th>Дата записи</th><th>Время записи</th><th>Номер кабинета</th></tr>";
                        for ($i = 0; $i < $rows; ++$i) {
                ...
                if ($query) {
                    ...
                    echo "<p>Выберите номер записи, которую хотите удалить</p>";
        ...
        if (!empty($_POST['theme'])) {
                $theme1 = htmlentities(mysqli_real_escape_string($link, $_POST['theme']));
                $query = "DELETE FROM schedule WHERE id='$theme1'";
                $queryTT = "DELETE FROM appointment_form WHERE id='$theme1'";
                
                $result1 = mysqli_query($link, $query) or die("Ошибка " . mysqli_error($link));
                $resultTT = mysqli_query($link, $queryTT) or die("Ошибка " . mysqli_error($link));
                if ($result1) {
